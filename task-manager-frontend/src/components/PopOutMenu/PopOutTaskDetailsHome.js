...
const PopOutTaskDetailsHome = ({ showSideTaskDetails, sideTaskDetails }) => {
  ...
  const [projectUsers, setProjectUsers] = useState(task.project.userProjects);
  const [assigneeUser, setAssigneeUser] = useState(task.account);
  ...
  const getProjectUsers = async (event) => {
    ...
    const userList = res.data.Users.filter((user) => {
      return user.id !== task.account.id;
    });
  ...
  const updateProject = async (e) => {
    ...
    const res = await apiServer.get(`/task/findByAssigneeId/${userId}`);
  ...
  const updateAssignee = async (e) => {
    ...
    const assignee = await apiServer.get(`/task/findById/${task.id}`);
  ...
  const handleCommentSubmit = async ({ text }) => {
    ...
    await apiServer.post(`/comment/save`, {
      text,
      userId,
      taskId
    });

    const comments = await apiServer.get(`/comment/findByTaskId/${task.id}`);
  ...
  const updateComplete = async () => {
    ...
    const updatedTask = await apiServer.post(`/task/completeTask/${task.id}`, {
      completed,
    });
    await taskdispatch({
    ...
    return (
      <div className="comment-container">
        <div className="comment-header">
          ...
          >
            {(comment.account.name[0] + comment.account.name[1]).toUpperCase()}
          </div>
