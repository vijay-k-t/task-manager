...
const AuthRoutes = () => {
  ...
  const getUserInfo = async () => {
    ...
    //   res = await apiServer.post("/account/login", { email });
  ...
  const getUserTeams = async () => {
    const id = localStorage.getItem("userId");
    const res = await apiServer.get(`/team/findByUserId/${id}`);
    await teamdispatch({ type: "get_user_teams", payload: res.data });
  };

  const getUserProjects = async () => {
    const id = localStorage.getItem("userId");
    const res = await apiServer.get(`/project/findByUserId/${id}`);
      ...
      <BrowserRouter>
        ...
        >
          <Switch>
            ...
            <Route exact path="/taskview" component={TasksPage} />
            <Route exact path="/taskstats" component={TaskStats} />
