...
const AddMemberForm = ({ teamId, clickClose, open, setTeamUsers }) => {
  ...
  const onSubmit = async ({ userId }) => {
    try {
      await apiServer.post(`/userteam/save`, {userId, teamId});
      const res = await apiServer.get(`/team/findById/${teamId}`);
      setTeamUsers(res.data.account);
  ...
  const getAllUsers = async () => {
    const res = await apiServer.get("/account/findAll");
    setUsers(res.data);
    setLoading(false);
  };
  useEffect(() => {
