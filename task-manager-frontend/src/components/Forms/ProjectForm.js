...
const ProjectForm = ({
  ...
  const onSubmit = async ({ name, teamId }) => {
    await apiServer.post(`/project/save/${userId}`, {
      name,
      teamId,
    });

    //REFER TO THIS WHEN CHECKING FOR RERENDERING
    const res = await apiServer.get(`/project/findByUserId/${userId}`);
    ...
    if (setTeamProjects) {
      const teamResponse = await apiServer.get(`/team/findById/${teamId}`);
