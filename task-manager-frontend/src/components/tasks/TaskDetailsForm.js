...
const TaskDetailsForm = ({
  ...
  const getTaskUser = async () => {
    const res = await apiServer.get(`/account/findByAssigneeId/${task.assignee_id}`);
    setUser(res.data);
    setLoading(false);
  };
  ...
  const onSubmit = async ({ name, due_date, description, completed }) => {
    ...
    try {
      ...
      const res = await apiServer.get(`/task/findByAssigneeId/${localStorage.getItem("userId")}`);
      await taskdispatch({ type: "update_task", payload: res.data });
      if (setTasklistTasks) {
        const taskres = await apiServer.get(`/tasklist/${task.tasklist_id}/tasks`);
